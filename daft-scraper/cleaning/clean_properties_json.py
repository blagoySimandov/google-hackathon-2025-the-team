...
def clean_property(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        ...
        cleaned = {
            ...
            'bedrooms': extract_number(listing.get('numBedrooms')) or listing.get('nonFormatted', {}).get('beds'),
            ...
            'seller': {
                ...
                'premierPartner': listing.get('seller', {}).get('premierPartnerSeller', False),
                'images': {
                    'profileImage': listing.get('seller', {}).get('profileImage'),
                    'profileRoundedImage': listing.get('seller', {}).get('profileRoundedImage'),
            ...
            'ber': {
                'rating': listing.get('ber', {}).get('rating') if listing.get('ber') else None,
            },
            ...
            'analytics': {
                'listingViews': item.get('props', {}).get('pageProps', {}).get('listingViews'),
            }
        }
...
def clean_properties_data(input_file: str, output_file: str):
    ...
    deduplicated.sort(key=lambda x: x.get('dates', {}).get('publishDate', ''), reverse=True)
...
def generate_summary(properties: List[Dict[str, Any]]):
    ...
    if property_types:
        print(f"\nTop Property Types:")
        for ptype, count in sorted(property_types.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {ptype}: {count}")
    ...
    with_images = sum(1 for p in properties if p.get('media', {}).get('totalImages', 0) > 0)
    print(f"\nMedia Statistics:")
    print(f"  Properties with images: {with_images} ({with_images/len(properties)*100:.1f}%)")
    print(f"  Properties with video: {sum(1 for p in properties if p.get('media', {}).get('hasVideo'))}")
    print(f"  Properties with virtual tour: {sum(1 for p in properties if p.get('media', {}).get('hasVirtualTour'))}")

    with_utilities = sum(1 for p in properties if p.get('extracted', {}).get('utilities'))
    print(f"\nExtracted Information:")
    print(f"  Properties with utility info: {with_utilities}")
    print(f"  Properties with Eircode: {sum(1 for p in properties if p.get('location', {}).get('eircodes'))}")
    print(f"  Properties with Folio: {sum(1 for p in properties if p.get('extracted', {}).get('folios'))}")
    print(f"  Properties with nearby locations: {sum(1 for p in properties if p.get('extracted', {}).get('nearbyLocations'))}")
